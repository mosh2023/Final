...
from app.core.errors import ORMObjectNoFoundError, ORMIdIsRequiredError, \
...
class BaseRepo(abc.ABC):
    ...
    @classmethod
    @abc.abstractmethod
    def get_repository(cls, orm: BaseModel, session: AsyncSession = async_session) -> BaseRepo:
        '''Factory for `Repository` row representation.'''

    @classmethod
    async def get(cls, id: int, session: AsyncSession = async_session) -> BaseRepo:
